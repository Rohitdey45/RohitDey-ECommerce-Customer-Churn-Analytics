"""Create a source-only, public-safe sibling folder without private outputs."""
from pathlib import Path
import shutil
import nbformat
root=Path.cwd()
dest=root.parent/(root.name+'-Public')
dest.mkdir(exist_ok=True)
for sub in ['scripts','data','outputs']:(dest/sub).mkdir(exist_ok=True)
for name in ['requirements.txt','.gitignore','data/README.md','scripts/create_documents.py','scripts/prepare_public_repository.py']:
    shutil.copy2(root/name,dest/name)
name='RohitDey_ECommerceCustomerChurnPrediction.ipynb'
notebook=nbformat.read(root/name,as_version=4)
for cell in notebook.cells:
    if cell.cell_type=='code':
        cell.outputs=[];cell.execution_count=None
nbformat.write(notebook,dest/name)
for name in ['model_metrics.csv','confusion_matrix.csv','quality_checks.csv','cleaning_audit.csv','pdf_extraction_audit.csv']:
    shutil.copy2(root/'outputs'/name,dest/'outputs'/name)
# Replace the private submission README: never claim the omitted files are bundled.
text='''# E-Commerce Customer Churn Prediction & Risk Analytics using RFM and Machine Learning
